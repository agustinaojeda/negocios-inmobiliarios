...
export default function PropertyCard({ property }) {
  ...
  const handleCardClick = () => {
    // Redirección al detalle de la propiedad
    window.location.href = ''; // Poner la ruta a DetalleInmueblePage
  };

  return (
    <article 
      ref={cardRef}
      className={`property-card reveal reveal-delay-${property.delay} ${isVisible ? 'visible' : ''}`} 
    ...
    >
      <div className="card-image-wrap">
        ...
        <span className="card-type">{property.operacionText}</span>
      </div>

      <div className="card-body">
        <div className="card-price">
          {property.priceFormatted}
          <span className="card-price-unit">{property.unit}</span>
        </div>
